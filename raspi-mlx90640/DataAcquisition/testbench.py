...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            for i in range(0, 15):
                data = str(plc.read_by_name("GVL_WtrSupCC.stZone11_PanelSup[{nPanelIndex}].fFlowrateAct".format(nPanelIndex=i), pyads.PLCTYPE_REAL))
                client.publish('testbench/flowrate/huba/{nPanelIndex}'.format(nPanelIndex=i), payload=data, qos=0, retain=False)
                print("send {data} to testbench/flowrate/huba/{nPanelIndex}".format(nPanelIndex=i, data=data))
        except:
            break
